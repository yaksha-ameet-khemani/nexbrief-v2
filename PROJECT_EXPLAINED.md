...
**Why we used it**: It's what turns the raw scraped article text into a short, readable summary, and separately, into a short "search query" used to build the "search this elsewhere" links. This project originally used the `llama-3.3-70b-versatile` model via Groq (carried over from the first version of the project). Groq retired that model in August 2026 — every call started returning HTTP 404, which silently froze the whole pipeline for ~16 days (see [Section 8.13](#813-a-retired-ai-model-silently-froze-the-entire-pipeline-for-two-weeks)) — so it was switched to `openai/gpt-oss-120b`, Groq's own recommended replacement, still on the free tier.
...
Every hour (and also whenever manually triggered for testing), the Worker runs a pipeline with several phases, loosely inspired by the original Java backend's design (with phases added since — normalization, translation-repair, auto-pause — that the original never needed). Note the phase *numbers* reflect the order they were added to the design, **not** the order they run in: since 2026-09-03, Phase 1 (fetch RSS) deliberately runs first (see [Section 7.16](#716-fetch-the-feeds-before-any-ai-work-so-a-broken-ai-provider-cant-starve-the-whole-pipeline)):

1. **Phase 1 — Fetch RSS (runs first).** Pull the latest items from all 6 RSS feeds, skipping any article URL we've already seen before (so the same article never gets processed twice) and skipping any source that's manually disabled *or* throttled to zero for having too deep a backlog (see [Section 7.10](#710-a-per-source-circuit-breaker-not-just-a-manual-switch)). This happens *before* any AI calls so that a failing AI provider can never eat the run's limited network-request budget before the feeds are even fetched.
2. **Phase -2 — Repair half-translated articles.** Scan already-"English" BBC Urdu articles for fields that still contain native script (a translation that silently failed on an earlier run) and retry just those fields.
3. **Phase -1 — Normalize any leftover non-English articles.** Scan the *entire* stored article list (not just pending ones) for any BBC Urdu article that hasn't been translated to English yet — whether it's still awaiting a summary or was already summarized before this behavior existed — and translate it in place. This is what let the ~50 already-cached Urdu articles catch up to English without writing a separate one-off migration script.
4. **Phase 0 — Clear the backlog.** Check for any articles from a *previous* run that got scraped but never got an AI summary (usually because Groq's rate limit was hit mid-run last time), skipping any source that's currently manually disabled. Try to finish summarizing those first.
5. **Phase 2 — Scrape.** For each genuinely new article from Phase 1, fetch its actual webpage and extract the full article text using source-specific rules (different news sites structure their HTML differently, so each source has its own set of CSS selectors to try, with generic fallbacks if those don't match). BBC Urdu's title, description, and content get translated to English here, before anything else touches them.
6. **Phase 3 — Summarize.** Send the extracted text to Groq, asking for a short summary (in English, even for BBC Urdu, since by this point the content has already been translated); then send the title + summary to Groq again, asking for a short search phrase, which is used to build a set of ready-made search links (different link sets depending on whether the article is about cricket, cars, tech, or general news). If Groq is unavailable this run — rate-limited *or* rejecting the request with a 4xx (e.g. a retired model) — this falls to Cloudflare Workers AI instead, and anything that still comes back empty just stays pending for a later run's Phase 0.
...
*(Superseded by 7.11 below — the hard on/off threshold described here was later replaced by a smoothly-tapering throttle, though the manual-vs-automatic distinction still holds.)*

### 7.11 A smooth fetch throttle instead of a hard on/off threshold

The threshold-based circuit breaker in 7.10 worked, but it's a blunt instrument: a source sits at full fetch speed right up until it crosses the line, then drops to zero all at once. It was replaced with a proportional version: each run, a source's fetch limit is computed as `MAX_ARTICLES_PER_SOURCE - currentPendingCount` (floored at zero) — so a source with a light backlog still gets close to its full fetch allowance, one that's getting backed up gets progressively less, and one that's over the limit gets none, all without a hard step-change. The single constant driving this is also now the same one the status page uses to decide whether to show a source as "auto-paused," which previously lived as two separately-tracked numbers that could in principle drift apart — a small consolidation, but one that removes a class of "the UI says X but the code does Y" bug before it could ever occur.

### 7.12 Two summarization lanes, so one rate limit doesn't stall everything

Even with the fetch throttle, Groq's free-tier rate limit is a *per-minute token budget*, not a per-source one — a single long article's summarization call can exhaust it, and once that happens, every other source's backlog also stalls until the next minute, regardless of how well-behaved any individual source's fetch rate is (this is also what 8.11 below is about). The fix: when Groq starts returning 429s partway through a run, the pipeline falls back to a second, independent AI provider — Cloudflare Workers AI — for the rest of that run's summarization work, rather than leaving those articles pending for a full hour. A shared flag (`groqState`) tracks Groq's availability for the whole run so both the backlog-clearing phase and the new-article phase agree on when to switch lanes, and every article records which lane actually summarized it (`summarizedBy: "groq" | "cloudflare"`), visible on the status page. In a live trial immediately after shipping this, Cloudflare ended up doing roughly three times Groq's volume during a stretch where Groq was frequently rate-limited — concrete evidence the fallback was pulling real weight, not just a theoretical safety net.

### 7.13 Translation accuracy needed a model with real-world knowledge, not just a language model

BBC Urdu's translate-before-everything design (7.9) initially used a *dedicated* neural machine translation model (`m2m100-1.2b`) — a model built specifically to translate text between languages, but with no broader knowledge of the world. This showed up as a real, visible problem: proper nouns it didn't recognize got translated as if they were ordinary words, phonetically or by best guess, producing headlines with garbled place and person names ("Strait of Hormuz" became "Horus," a cricketer's name came out as a completely different name). Swapping in a general-purpose *instruction-following* LLM (prompted specifically to preserve names and places) fixed the grammar and fluency problems entirely, but didn't fully fix the name accuracy — a language model can still confidently produce a wrong-but-plausible-sounding name instead of an obviously garbled one, which is arguably a harder failure mode to catch, not an easier one. The fix that actually resolved it was routing translation through the same large, more capable model already used for summarization (Groq's 70-billion-parameter model), which has enough real-world knowledge to recognize and correctly transliterate most proper nouns it encounters, with the smaller Cloudflare model only as a fallback when Groq itself is unavailable that run.

The broader lesson mirrored here: a smaller model purpose-built for a narrow task (translation) isn't automatically better at that task than a larger, more general model — when the task quietly depends on world knowledge (here, knowing that a particular string of Urdu characters is a specific person's name and not a common phrase), a bigger, more broadly-trained model can outperform a specialized one, and there's no way to know which effect dominates without actually comparing live output side by side.

### 7.14 Detecting a translation that silently didn't happen

A quieter bug sat underneath 7.13's fixes: both translation providers could occasionally hand back the *original* native-script text unchanged (or only partly translated) instead of raising an error, and the code that called them only checked "did I get a non-empty string back" — which a no-op failure satisfies just as well as a real translation does. Since a successful-looking call flipped `article.language` to `"en"` regardless, and the self-healing migration pass (7.9) only ever retries articles *not* already marked English, an article that failed silently this way became permanently stuck showing raw Urdu — invisible to the exact mechanism built to catch translation failures, because from that mechanism's point of view, it had already succeeded. The fix adds an actual verification step after each translation attempt: a simple check for whether the result still contains characters from the source script (Arabic-block characters for Urdu, Devanagari for Hindi) using Unicode character ranges — if it does, the translation is treated as failed regardless of what the API claimed, and a separate repair pass specifically re-scans already-"English" articles for this exact symptom and retries just the fields that are still broken.

**Lesson**: "the API call returned successfully" and "the API call did what I asked" are different claims, and conflating them is a recurring shape of bug across this whole project (see also 8.9's "the operation finished" vs. "the result was saved") — anywhere an external system can plausibly return a well-formed but semantically wrong answer, it's worth adding a real check on the *content* of the response, not just its shape or presence.

### 7.15 Matching a reference design by comparing actual computed styles, not eyeballing a screenshot

When asked to make the homepage's look closer to a reference template, the first temptation is to eyeball both screenshots side by side and guess at what's different ("the fonts seem off," "something about the spacing"). Instead, the reference site was opened directly in a browser and its actual rendered CSS was read back programmatically — exact font families, sizes, weights, colors, padding, and border values for the specific elements in question (the category tags, the nav links, the headline text) — and compared against the same measurements taken from the live NexBrief site. This turned a vague "make it look more like the template" into a short list of concrete, checkable deltas: the reference's category tag was a solid-filled rectangle with zero border-radius, not an outlined translucent pill; its two smaller headlines were set at the exact same font size as its main headline, not a noticeably smaller one; its secondary-story images ran edge-to-edge with padding only around the text, not padded uniformly on all sides. Each of those became a small, targeted code change instead of a round of guessing followed by "does this look righter now?"

**Lesson**: when the goal is to visually match something that already exists and is inspectable, reading its actual computed styles (via a browser's own rendering engine) is strictly more reliable than comparing screenshots by eye — it replaces subjective impressions with numbers that can be diffed directly.

### 7.16 Fetch the feeds before any AI work, so a broken AI provider can't starve the whole pipeline

For most of the project's life the pipeline ran its phases in the order they were invented: first the translation-repair and normalize passes (which make a lot of AI calls), then backlog clearing (more AI calls), and only then the actual fetch of new RSS items. That ordering hid a landmine. A Cloudflare Worker invocation is allowed only a limited number of outbound network requests (50 on the free plan), shared across *everything* the run does — AI calls and RSS fetches alike. As long as the AI calls succeeded quickly this was fine. But when every AI call started failing and retrying (see [Section 8.13](#813-a-retired-ai-model-silently-froze-the-entire-pipeline-for-two-weeks)), the early phases burned the entire request budget before the fetch step ran — so no new articles were ever discovered, for over two weeks, with no outward sign anything was wrong.

The fix was to reorder the pipeline so **fetching new RSS items happens first, before any AI work**. New articles are cheap to fetch, they're the one thing that genuinely can't be recovered later (a feed only lists its most recent items — miss them and they're gone), and everything downstream (scraping, summarizing) already tolerates being deferred to a later run. So even if the AI phases later exhaust the budget or time out, the day's news is already safely captured.

**Lesson**: when several pieces of work share one exhaustible budget and can't all be guaranteed to finish, do the irreplaceable, can't-be-retried-later work *first*. Ordering by "what breaks worst if it doesn't happen" beats ordering by "what was written first" or "what's logically upstream."
...
### 8.10 Filters that were supposed to work together were silently replacing each other

The user noticed something subtle just by using the site: the default "All" view had no scrollbar (not much content), but clicking the "Cricket" category filter suddenly showed enough content to scroll — an inconsistency with no obvious cause from the UI alone. The actual backend filtering code used a chain of `if / else if` checks: if a keyword search was active, use *only* that; otherwise if a source was picked, filter by *only* that; otherwise if a category was picked, filter by *only* that; otherwise fall back to filtering by date. This structure meant these filters were being treated as mutually exclusive alternatives rather than combinable constraints — picking a category silently discarded the date filter entirely (showing that category's articles across the *entire* multi-day retention window), while the plain "All" view (no category/source/keyword active) was the only case that ever got date-filtered at all. Two views that looked like they should behave consistently ("all articles" vs. "just the cricket ones") were actually operating under completely different rules, invisibly. This exact `if / else if` structure had been ported directly from the original Java backend's query logic, without anyone questioning whether the underlying behavior was actually sensible — it likely had the same inconsistency, just never surfaced or noticed there.

The fix: rewrite the filtering as a sequence of independent, optional conditions — if a keyword is given, apply it; if a source is given, apply it; if a category is given, apply it; if a date is given, apply it — each one narrowing down whatever the previous ones already narrowed down, rather than one replacing all the others. Combined with [Problem 7.7](#77-show-everything-cached-by-default-the-date-picker-is-opt-in-not-a-default-filter)'s change to stop defaulting to "today," this made every combination of filters behave predictably and consistently with each other.
...
### 8.12 A plausible-sounding Workers AI model ID turned out to be deprecated

Adding Cloudflare Workers AI as a summarization fallback (7.12) meant picking a specific model ID to call. The first choice, `@cf/meta/llama-3.1-8b-instruct` (no suffix), looked like a completely reasonable, standard model name — and every single call to it failed immediately with an error code, caught right away by watching live logs (`wrangler tail`) rather than waiting for a user report. It turned out that specific model had been deprecated as of a date shortly before this was written, in favor of a `-fast` variant of the same model. The fix was simply using the correct, current model ID — but the mistake itself is a useful data point: a model name can be syntactically valid, semantically plausible, and still refer to something that no longer exists or has been superseded, and neither TypeScript nor the code's own logic has any way to know that ahead of time.

**Lesson**: when integrating with an AI provider's model catalog, a model ID "looking right" isn't the same as it being current — checking the provider's own up-to-date model list or docs page before relying on a specific ID is worth the extra minute, especially for fast-moving catalogs like Workers AI's, and watching live logs immediately after a change is how this got caught in minutes rather than silently failing for a while.

### 8.13 A retired AI model silently froze the entire pipeline for two weeks

Around August 18, 2026, the site stopped getting any new articles. Nothing had been deployed; no code had changed in a month. The `/status` page looked completely healthy — it showed the pipeline running every hour, right on schedule.

The cause: **Groq retired the AI model this project had always used** (`llama-3.3-70b-versatile`). Groq had announced the retirement back in June; from mid-August, every request to that model came back as an HTTP 404 error ("not found"). That one dead model cascaded into a total outage through three compounding weaknesses:

1. **The retry logic treated "model doesn't exist" like a temporary glitch.** When a Groq call failed, the code retried it 3 times with growing pauses (5 seconds, then 10) before giving up — sensible for a brief network hiccup, useless for a model that is never coming back. Worse, only one *specific* kind of failure (a rate-limit response) told the pipeline "stop using Groq for the rest of this run and use the backup instead." A 404 wasn't that kind, so the pipeline kept trying Groq — and burning those 15-second retry cycles — for *every single article and every translation*, run after run.

2. **Those wasted retries used up a hidden per-run budget.** A Cloudflare Worker is allowed only a limited number of outbound requests per run — 50 on the free plan — shared across everything: AI calls *and* the RSS feed fetches. The translation-repair and cleanup steps ran first and made dozens of doomed Groq calls, exhausting the budget before the pipeline ever got to the step that fetches new articles. So every feed fetch failed with "too many requests," and zero new articles were found — every hour, for over two weeks.

3. **The health check only proved the run *ended*, not that it *did anything*.** The "last run" timestamp on the status page is written at the very end of a run. The run was reaching the end (having accomplished nothing), so the timestamp kept updating and everything looked fine. The failure was only visible by streaming the live logs (`wrangler tail`) and watching the `HTTP error: 404` messages pile up followed by "too many requests."

The fix had three parts: **(1)** point the config at Groq's recommended replacement model (`openai/gpt-oss-120b`); **(2)** treat *any* "your request was wrong" response from Groq (a bad API key, a missing model — the whole 4xx family, except rate-limits) as final, not worth retrying, and have it immediately switch the run over to the Cloudflare backup — the same way a rate-limit already did; and **(3)** reorder the pipeline so fetching new articles happens *first*, before any AI work, so this class of failure can never again starve the one step that can't be caught up later (see [Section 7.16](#716-fetch-the-feeds-before-any-ai-work-so-a-broken-ai-provider-cant-starve-the-whole-pipeline)).

**Lessons** (several, and most of them are echoes of earlier problems in this very document):
- **"The call came back" and "the call did what I asked" are different claims** — a 404 response *is* a response; code that only distinguishes "responded" from "threw an exception" will loop forever on a permanent, well-formed failure. (Same shape as [8.9](#89-a-background-task-was-silently-getting-killed-and-throwing-away-completed-work) and [7.14](#714-detecting-a-translation-that-silently-didnt-happen).)
- **Retry only what retrying can fix.** "Wait and try again" is right for a timeout or a 5xx server error; it's actively harmful for a 4xx, which means *the request itself* is wrong and will be wrong every time.
- **A dependency you don't control will be discontinued on someone else's schedule.** Third-party model catalogs churn (this is the *second* model-retirement in this project — see [8.12](#812-a-plausible-sounding-workers-ai-model-id-turned-out-to-be-deprecated)); the resilient design isn't "pick a model that won't be retired," it's "fail over cleanly to a backup when one is."
- **A health signal that can't tell "working" from "running but doing nothing" is worse than no signal** — it actively delays discovery. A useful check would assert *"the last run fetched or summarized at least something"*, not merely *"a run finished."*

### 8.14 The same story showed up twice because one source changed its own URL between runs

The user noticed duplicate articles in the list — the same headline appearing twice. All the duplicates were from one source, Dainik Bhaskar. Comparing the two copies of each pair showed they were genuinely the same story: identical headline, identical publish time, identical trailing ID number in the link. The only difference was the link itself — one copy's URL had an extra `/g/` segment near the start (`bhaskar.com/g/national/news/…`) and the other didn't (`bhaskar.com/national/news/…`).

The pipeline avoids re-adding an article it already has by checking whether the article's URL is already in storage — an exact, character-for-character string match. That works as long as a given story always has exactly one URL. But Bhaskar's RSS feed serves the *same* story under both URL forms and flips between them from one hourly run to the next: one run's feed lists the story with `/g/`, the next run's lists it without. To the exact-match check, the second form is a URL it has never seen, so the story gets treated as brand new — fetched, scraped, and summarized a second time (a wasted AI call), and both copies end up on the site. The giveaway was in the timestamps: the two copies of each pair were created exactly one hour apart, i.e. on consecutive runs.

The fix has two halves, both deliberately scoped to Bhaskar alone (every other source has stable, single-form URLs and is left completely untouched): **(1)** before comparing or storing a Bhaskar URL, strip the optional `/g/` segment so both forms collapse to one canonical spelling — now the second form matches the first and the duplicate is never created; **(2)** a one-time cleanup pass, which also runs on every future pipeline run as a safety net, that scans the existing stored articles, finds any Bhaskar entries that are the same story under the two URL forms, and keeps just one of each (preferring the copy that already has a finished AI summary). Shipped September 4, 2026; the next hourly run collapsed the six existing duplicate pairs and rewrote the affected URLs to the canonical form.

**Lesson**: "the URL is the unique ID for a thing" is an assumption worth being suspicious of when the URL comes from someone else's system — a site can and will serve the same content at more than one address, and change which one it advertises without notice. Deduplicating on a *normalized* form of the identifier (or on a more stable field, like the numeric story ID embedded in Bhaskar's links) is more robust than trusting the raw string. The fix was kept as narrow as possible — one source, one known URL quirk — rather than normalizing every source's URLs and risking a new problem somewhere that was working fine.
...
- **Subrequest**: Any outbound network request a serverless function makes while handling one invocation (an API call, fetching a webpage, etc.). Platforms cap how many are allowed per invocation — Cloudflare Workers' free plan allows 50, the paid plan 1000 — and this cap is shared across *every* outbound call the run makes. Exhausting it makes all further requests in that run fail. See [Section 8.13](#813-a-retired-ai-model-silently-froze-the-entire-pipeline-for-two-weeks).
...
- **A model built specifically for a narrow task isn't automatically better at it than a larger, general-purpose one** — a dedicated translation model with no world knowledge produced worse real-world results than a general LLM, because the task quietly depended on recognizing proper nouns, not just grammar (see [Section 7.13](#713-translation-accuracy-needed-a-model-with-real-world-knowledge-not-just-a-language-model)).
- **A "successful-looking" result and a "correct" result are different claims, and code that only checks the first one can create bugs that are invisible to its own retry logic** — a translation that silently failed but still returned a well-formed string looked identical to a real success, which meant the exact mechanism built to catch failures couldn't see this particular kind of failure at all (see [Section 7.14](#714-detecting-a-translation-that-silently-didnt-happen)).
- **When matching an existing visual reference, read its actual computed CSS instead of eyeballing a screenshot** — this turns "make it look more like X" into a short list of concrete, verifiable numbers (font sizes, colors, padding) instead of a round of guesswork (see [Section 7.15](#715-matching-a-reference-design-by-comparing-actual-computed-styles-not-eyeballing-a-screenshot)).
- **Only retry what retrying can fix** — a timeout or a 5xx server error deserves a retry; a 4xx (bad request, missing model, dead key) means the request itself is wrong and will be wrong every time, so retrying it just wastes time and budget while hiding the real problem (see [Section 8.13](#813-a-retired-ai-model-silently-froze-the-entire-pipeline-for-two-weeks)).
- **A monitoring signal that can't distinguish "working" from "running but doing nothing" is worse than nothing** — it delays discovery of an outage instead of surfacing it. "A run finished" is a weak check; "the last run actually fetched or produced something" is a useful one.
- **A third-party dependency will be discontinued on its owner's schedule, not yours** — external model catalogs, APIs, and services get retired regularly; resilience comes from failing over cleanly to a fallback, not from betting on a particular one sticking around.
- **When work items share one exhaustible budget and might not all finish, do the irreplaceable ones first** — new articles from an RSS feed can't be fetched later (the feed moves on); a summary can always be generated on the next run. Order by "what's unrecoverable if skipped," not by "what came first in the code" (see [Section 7.16](#716-fetch-the-feeds-before-any-ai-work-so-a-broken-ai-provider-cant-starve-the-whole-pipeline)).
- **An identifier that comes from someone else's system isn't guaranteed to be stable or unique** — one source served the same story at two different URLs and alternated between them run to run, so deduplicating on the exact URL string silently created duplicates. Match on a normalized form of the identifier (or a more stable field) when the raw value isn't under your control (see [Section 8.14](#814-the-same-story-showed-up-twice-because-one-source-changed-its-own-url-between-runs)).
